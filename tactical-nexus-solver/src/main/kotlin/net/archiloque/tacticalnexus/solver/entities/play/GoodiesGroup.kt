...
class GoodiesGroup(
    ...
    val item = Item(
        ...
        items.sumOf { it.inputItem.expMultAdd },
        items.map { it.inputItem.expMultMul }.fold(1) { acc, i -> acc * i },
        items.sumOf { it.inputItem.hp },
        items.sumOf { it.inputItem.hpMultAdd },
        items.map { it.inputItem.hpMultMul }.fold(1) { acc, i -> acc * i },
    )
